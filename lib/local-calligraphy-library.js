...
import { normalizeCalligraphyText } from "@/lib/calligraphy";
...
function chooseGlyphForChar(char, charIndex, glyphIndex) {
  const candidates = [char, variantChars[char], normalizeCalligraphyText(char)].filter(Boolean);

  for (const candidate of [...new Set(candidates)]) {
    const glyphId = chooseGlyphId(charIndex[candidate]);
    if (glyphId && glyphIndex.has(glyphId)) return glyphIndex.get(glyphId);
  }
