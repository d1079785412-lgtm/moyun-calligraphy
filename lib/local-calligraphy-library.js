...
function chooseGlyphForChar(char, charIndex, glyphIndex) {
  const directId = chooseGlyphId(charIndex[char]);
  if (directId && glyphIndex.has(directId)) return glyphIndex.get(directId);

  const variant = variantChars[char];
  const variantId = variant ? chooseGlyphId(charIndex[variant]) : null;
  if (variantId && glyphIndex.has(variantId)) return glyphIndex.get(variantId);
...
export function tryGenerateLocalCalligraphy({ text, script, master, format, charsPerLine, prompt }) {
  ...
  for (const char of chars) {
    const glyph = chooseGlyphForChar(char, charIndex, glyphIndex);
    if (!glyph) {
