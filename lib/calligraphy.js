...
export const displaySceneModes = {
  framed: "正常装框",
  living: "客厅装框",
  gallery: "展馆装框",
};
...
export function buildImagePrompt({ text, script, master, format }) {
  ...
  return [
    ...
    "反向约束：不要生成错别字，不要生成乱码，不要生成多余文字，不要把字写成印刷字体，不要把不同书体混在一起，不要让文字被裁切、重叠、遮挡或变形，不要出现现代海报字、卡通字、霓虹字、英文签名或 AI 水印。",
  ].join("\n");
}

export function buildDisplayScenePrompt({ text, script, master, format, sceneMode }) {
  const sceneName = displaySceneModes[sceneMode] || displaySceneModes.framed;
  const formatGuide = formatGuides[format] || formatGuides.中堂;
  const baseArtwork =
    `画面核心是一幅已经完成的中国传统书法作品，内容为“${text}”，书体为${script}，风格参考${master}，作品形式为${format}。` +
    `必须优先使用随附的参考作品图作为装框对象，保留参考图中的字形、章法比例、落款和朱红印章，不要重新设计正文，不要替换字形。` +
    `作品要保持中文字形准确，宣纸质感，黑色墨迹，传统落款与朱红印章，不要生成乱码，不要添加无关文字。`;

  if (sceneMode === "living") {
    return `${baseArtwork} 请把参考作品图完整放入画框，不裁切正文，不改变扇面、横幅、条幅等原始外形。生成一张真实感室内效果图：书法作品被素雅木质或黑色细框装裱，悬挂在现代中式客厅主墙，墙面干净，沙发、茶几、柔和自然光作为环境参照。作品比例符合${format}特点：${formatGuide} 画面真实、端正、适合向书法爱好者展示装饰效果。`;
  }

  if (sceneMode === "gallery") {
    return `${baseArtwork} 请把参考作品图完整放入画框，不裁切正文，不改变扇面、横幅、条幅等原始外形。生成一张真实感展馆展陈效果图：书法作品被专业装框，悬挂在白色或浅灰展墙上，有展馆射灯、作品标签、安静的展陈空间。作品比例符合${format}特点：${formatGuide} 画面干净高级，像大学生项目答辩中的艺术展览预览图。`;
  }

  return `${baseArtwork} 请把参考作品图完整放入画框，不裁切正文，不改变扇面、横幅、条幅等原始外形。生成一张真实感正常装框效果图：书法作品平整装裱在素雅画框中，正面视角，浅色背景，能清楚看到宣纸、墨色、落款和印章。作品比例符合${format}特点：${formatGuide} 不要出现复杂房间背景，重点展示装框后的作品本身。`;
}
