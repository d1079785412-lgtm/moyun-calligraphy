...
const scriptPromptGuides = {
  楷书: "楷书要求点画分明、结构端正、起收清楚，避免印刷宋体或电脑字体感。",
  行书: "行书要求笔势连贯、牵丝自然、行气流动，但每个中文字仍需清晰可辨。",
  草书: "草书要求有书法性的连绵与使转，保持传统草法意趣，避免生成无法辨识的乱码或涂鸦。",
  隶书: "隶书要求横画舒展，体现蚕头燕尾、波磔分明和古朴扁方的结体特征。",
  篆书: "篆书要求采用小篆/篆书风格，线条圆转匀净，结体纵长对称，可参考古文字形态，但不要生成现代简体字或乱码。",
};

const scriptNegativeGuides = {
  楷书: "不要生成宋体、黑体、印刷体、过度装饰字体。",
  行书: "不要生成硬笔连笔字、英文花体或难以辨认的连线。",
  草书: "不要生成抽象涂鸦、非中文字形、英文符号或随机线条。",
  隶书: "不要生成楷书化隶书、现代美术字或缺少波磔的扁平字体。",
  篆书: "不要生成甲骨文乱码、符号图案、现代简体结构或无法识别的伪篆字。",
};
...
export function buildImagePrompt({ text, script, master, format }) {
  return `生成一幅中国传统书法作品，内容为“${text}”，必须使用繁体中文/正体中文字形，书体为${script}，风格参考${master}，作品形式为${format}。${getScriptPromptGuide(script)}使用宣纸质感，黑色墨迹，自然飞白，传统落款，朱红印章。整体章法端正，墨色有浓淡变化，画面干净，适合书法艺术展示。注意保持中文字形准确，不要生成乱码，不要改成简体字，不要添加无关文字。`;
}
