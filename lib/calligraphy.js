...
export function buildDisplayScenePrompt({ text, script, master, format, sceneMode }) {
  ...
  if (sceneMode === "living") {
    return `${baseArtwork} 请生成一张真实感室内效果图：书法作品被素雅木质或黑色细框装裱，悬挂在现代中式客厅主墙，墙面干净，沙发、茶几、柔和自然光作为环境参照。作品比例符合${format}特点：${formatGuide} 画面真实、端正、适合向书法爱好者展示装饰效果。`;
  }

  if (sceneMode === "gallery") {
    return `${baseArtwork} 请生成一张真实感展馆展陈效果图：书法作品被专业装框，悬挂在白色或浅灰展墙上，有展馆射灯、作品标签、安静的展陈空间。作品比例符合${format}特点：${formatGuide} 画面干净高级，像大学生项目答辩中的艺术展览预览图。`;
  }

  return `${baseArtwork} 请生成一张真实感正常装框效果图：书法作品平整装裱在素雅画框中，正面视角，浅色背景，能清楚看到宣纸、墨色、落款和印章。作品比例符合${format}特点：${formatGuide} 不要出现复杂房间背景，重点展示装框后的作品本身。`;
}
