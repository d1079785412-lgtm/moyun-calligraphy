...
export async function generateSceneImage({ prompt, format, script, referenceImageUrl }) {
  const config = getImageModelConfig();
  ...
  if (config.provider === "qwen") {
    return callQwenImage({
      ...config,
      prompt,
      size: imageSizeForScene(format),
      script,
      referenceImageUrl,
      negativePrompt:
        "低清晰度，低质量，乱码，错别字，错误中文，多余文字，水印，过度装饰，画框歪斜，作品变形，重写书法正文，替换参考图字形，改变参考图章法，室内杂乱，人物特写。",
    });
  }
...
function imageSizeForScene(format) {
  if (format === "横幅" || format === "对联") return "1664*928";
  return "1328*1328";
}
