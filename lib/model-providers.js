import { buildLayoutAdvice, mockChatAnswer } from "@/lib/calligraphy";
...
export async function generateCalligraphyImage({ prompt, format }) {
  ...
  if (config.provider === "qwen") {
    return callQwenImage({ ...config, prompt, size: imageSizeForFormat(format) });
  }
...
async function callQwenImage({ provider, baseUrl, apiKey, model, prompt, size }) {
  if (!apiKey) {
  ...
  const response = await fetch(
    `${baseUrl.replace(/\/$/, "")}/services/aigc/multimodal-generation/generation`,
    {
      ...
      body: JSON.stringify({
        ...
        parameters: {
          negative_prompt: "低清晰度，低质量，乱码，错别字，多余文字，现代印刷字体，水印，杂乱背景，边框破损，过度装饰",
          prompt_extend: false,
          watermark: false,
          size,
          n: 1,
