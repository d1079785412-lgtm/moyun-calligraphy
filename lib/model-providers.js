...
function getPoemModelConfig() {
  const textConfig = getTextModelConfig();
  return {
    ...textConfig,
    provider: "deepseek",
    baseUrl: process.env.POEM_MODEL_BASE_URL || process.env.TEXT_MODEL_BASE_URL || "https://api.deepseek.com",
    apiKey: process.env.POEM_MODEL_API_KEY || process.env.TEXT_MODEL_API_KEY || "",
    model: process.env.POEM_MODEL_NAME || "deepseek-v4-pro",
  };
}
...
export async function generatePoemText({ keywords, genre }) {
  const config = getPoemModelConfig();
  ...
  if (config.provider === "deepseek") {
      ...
      messages: [
        {
          role: "system",
          content:
            "你是原创古典诗文创作助手。请按用户关键词生成适合书法作品的中文正文。必须原创，不要引用、照搬、改写、拼接任何古代诗人的现成诗句。只输出正文，不要解释，不要标题，不要引号。五言绝句输出两行，每行格式为五个汉字，一个中文逗号，再五个汉字。七言绝句输出两行，每行格式为七个汉字，一个中文逗号，再七个汉字。",
        },
        {
          role: "user",
          content: `关键词：${cleanKeywords.join("，") || "清雅"}。体裁：${cleanGenre}。请生成原创正文，避免古人成句。`,
        },
      ],
    });
    ...
    return {
      provider: result.provider,
      text: formatPoemText(result.answer, cleanGenre),
    };
  }

  throw new Error("成诗功能请配置 DeepSeek V4 Pro。");
}
...
function formatPoemText(value, genre) {
  const maxChars = genre === "七言绝句" ? 28 : 20;
  const half = genre === "七言绝句" ? 7 : 5;
  const fallback = genre === "七言绝句"
    ? "清风一榻生秋意明月半窗照墨痕山色不言留古韵诗心随笔到柴门"
    : "清风入静夜明月照闲庭墨气生秋水诗心寄远星";
  const text = `${String(value || "")
    .normalize("NFKC")
    .replace(/[^\p{Script=Han}]/gu, "")}${fallback}`
    .slice(0, maxChars);
  return [
    `${text.slice(0, half)}，${text.slice(half, half * 2)}`,
    `${text.slice(half * 2, half * 3)}，${text.slice(half * 3, half * 4)}`,
  ].join("\n");
}

function poemArtworkText(value) {
  return String(value || "")
    .normalize("NFKC")
